...
class Game {
 ...
 allObjects() {
  ...
  detectCollision(player) {
    ...
    this.coins.forEach(coin => {
      if(Util.inSamePos(coin, player)) {
        ...
        if(this.score % 2 == 0) {
          this.level += 1;
          this.beginNewLevel(LEVELS[this.level]);
        } else {
          this.spawnCoin();
        }
  ...
  beginNewLevel(level) {
    ...
    this.cube.css("-webkit-transform", "rotateX(0deg) rotateY(0deg)");

    this.cube.data('v-rot', 0);
    this.cube.data('h-rot', 0);
...
let e1 = new Enemy({face: "front", pos: [0,1], direction:[0,1]});
let e2 = new Enemy({face: "front", pos: [0,1], direction:[0,1]});
let e3 = new Enemy({face: "front", pos: [0,1], direction:[0,1]});
let e4 = new Enemy({face: "front", pos: [0,1], direction:[0,1]});
